...
这是一个基于Python和React的数学学习项目，专注于生成100以内的加减法口算练习题。项目采用前后端分离架构，后端使用FastAPI提供RESTful API服务，前端使用React构建用户界面，支持在线预览和Word文档下载功能。
...
核心生成器负责创建符合教育要求的数学问题，确保所有计算结果都在100以内且满足基本的数学规则。
...
专门负责将生成的数学问题转换为格式化的Word文档，支持自定义标题和页面布局。
...
FastAPI提供了高性能的RESTful API服务，支持CORS跨域访问和静态文件托管。
...
React组件提供了直观的用户界面，支持实时配置和预览功能。

```mermaid
flowchart TD
Start([用户打开应用]) --> Config[配置参数]
Config --> Range[调整题目数量<br/>10-100题]
Config --> Ops[选择运算类型<br/>加法/减法]
Ops --> Generate[点击生成按钮]
...
### 功能特性
- **教育导向**：专注于100以内加减法的实用练习
- **用户体验**：直观的React界面和流畅的交互体验
- **输出多样化**：支持在线预览和Word文档下载
- **配置灵活**：可调节题目数量和运算类型
...
该项目为数学教育应用提供了一个优秀的参考实现，代码结构清晰、功能完整，适合进一步的功能扩展和维护。